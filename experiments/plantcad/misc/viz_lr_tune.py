...
def fetch_wandb_runs(project_name: str | None = None, run_pattern: str = "plantcad-lr-tune-lr.*-r5"):
...
def list_available_runs(project_name: str | None = None, limit: int = 20):
...
def extract_metrics_data(runs: Any, metrics: list[str] | None = None):
    """Extract step-by-step metrics data from W&B runs."""
    if metrics is None:
        metrics = ["eval/loss", "throughput/mfu", "train/loss"]
    all_data = []
        ...
        for metric in metrics:
            ...
            for row in history:
                if metric in row and "_step" in row:
                    all_data.append({"run_name": run_name, "metric": metric, "step": row["_step"], "value": row[metric]})
...
def create_visualizations(df):
    ...
    plot_original = (
        ...
        + theme_minimal()
        + labs(title="PlantCAD Learning Rate Tuning - Metrics by Step", x="Training Step", y="Metric Value", color="Run")
