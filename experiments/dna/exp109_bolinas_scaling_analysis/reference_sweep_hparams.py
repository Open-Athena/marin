...
"""Visualize reference sweep hparams vs eval/loss with feature importance.

Usage:
    uv run --with scikit-learn experiments/dna/exp109_bolinas_scaling_analysis/reference_sweep_hparams.py [--refresh]
...
def fetch_data(project: str, group: str) -> list[dict]:
    ...
    for r in runs:
        if r.state != "finished":
            continue
        loss = r.summary.get("eval/loss")
        macro = r.summary.get("eval/macro_loss")
        ...
        data.append(
            {"loss": loss, "macro_loss": macro, "name": r.name, "completed_at": r.heartbeatAt, "loop": loop, **hparams}
        )
    return data
...
def plot(data: list[dict]) -> None:
    ...
    model = GradientBoostingRegressor(
        n_estimators=100, max_depth=3, learning_rate=0.1, min_samples_leaf=2, random_state=42
    ).fit(X, y)
    ...
    ax_time.set_ylabel("eval/loss", fontsize=9)
    ...
    for i, (hparam, label) in enumerate(zip(HPARAM_KEYS, HPARAM_LABELS, strict=True)):
        ...
        if hparam in LOG_SCALE:
            ax.set_xscale("log")
        if col == 0:
            ax.set_ylabel("eval/loss", fontsize=9)
        else:
            ax.set_yticklabels([])

    fig.suptitle(f"Bolinas DNA reference sweep v0.6 ({len(data)} finished runs)", fontsize=11, y=0.98)
